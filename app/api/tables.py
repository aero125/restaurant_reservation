...
class TablesService(DatabaseManager):
    def get_tables(self, limit: int, skip: int) -> List[Table]:
        ...
        try:
            return [Table(id=row[0], table_number=row[1], price=row[2], seats=row[3]) for row in rows]
    ...
    def delete_table(self, table_number: int) -> dict:
        ...
        rows = self.execute_query(query, params, fetch=True)
        if not rows:
            raise HTTPException(status_code=404, detail="Table with this table_number was not found")
        table_id = rows[0][0]

        conn = self.get_connection()
        try:
            with conn.cursor() as cursor:
                cursor.execute('SELECT user_id, price FROM reservations WHERE table_id = %s', 
                            (table_id, ))
                ...
                cursor.execute('DELETE FROM tables WHERE id = %s RETURNING id',
                            (table_id, ))
                row = cursor.fetchone()
                table_id = row[0]
                conn.commit()
...
def get_tables(limit: Annotated[int, Query(gt=0, le=1000)] = 10, skip: Annotated[int, Query(ge=0, le=1000)] = 0):
